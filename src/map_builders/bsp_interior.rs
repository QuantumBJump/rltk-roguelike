...
impl BspInteriorBuilder {
    ...
    fn build(&mut self, rng: &mut RandomNumberGenerator, build_data: &mut BuilderMap) {
        ...
        // Populate the shared state with the list of generated rooms
        build_data.rooms = Some(rooms);
    }

    fn add_subrects(&mut self, rect: Rect, rng: &mut RandomNumberGenerator) {
        // Remove the previous rect from the list
        ...
        if split <= 2 {
            //Horizontal split
            // Create left half room
            let h1 = Rect::new( rect.x1, rect.y1, half_width-1, height );
            self.rects.push(h1);
            // Further subdivide if possible
            if half_width > MIN_ROOM_SIZE { self.add_subrects(h1, rng); }
            
            // Create right half room
