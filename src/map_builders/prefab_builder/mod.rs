use super::{
    MapBuilder, Map, TileType, Position, SHOW_MAPGEN_VISUALISER,
    get_most_distant_area
};
...
impl PrefabBuilder {
    ...
    fn apply_room_vaults(&mut self) {
        ...
        // TODO: This is a placeholder & will be moved out of this function
        let master_vault_list = vec![TOTALLY_NOT_A_TRAP, CHECKERBOARD, SILLY_SMILE];

        // Filter the vault list down to ones that are applicable at the current depth
        let mut possible_vaults: Vec<&PrefabRoom> = master_vault_list
            .iter()
            .filter(|v| { self.depth >= v.first_depth && self.depth <= v.last_depth })
            .collect();

        if possible_vaults.is_empty() { return; } // Bail out if there are no valid rooms
        ...
        for _i in 0..n_vaults {
            ...
            // Make a list of places the vault could fit
            ...
            loop {
                ...
                // Check that we don't overflow the map
                if x > 1
            ...
            if !vault_positions.is_empty() {
                // If we could place the vault somewhere, choose where to place it.
                ...
                // Don't spawn entities in the prefab room
