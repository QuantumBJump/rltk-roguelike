...
impl YellowBrickRoad {
    ...
    fn find_exit(&self, build_data: &mut BuilderMap, seed_x: i32, seed_y: i32) -> (i32, i32) {
        ...
        let end_x = available_floors[0].0 as i32 % build_data.width;
        let end_y = available_floors[0].0 as i32 / build_data.width;
        (end_x, end_y)
    }

    fn paint_road(&self, build_data: &mut BuilderMap, x: i32, y: i32) {
        if x < 1 || x > build_data.map.width-2 || y < 1 || y > build_data.height-2 {
            return; // Can't paint a road outside the bounds of the map!
        }
    ...
    fn build(&mut self, rng: &mut rltk::RandomNumberGenerator, build_data: &mut BuilderMap) {
        ...
        let (seed_x, seed_y, stream_startx, stream_starty) = if exit_dir == 1 {
            (build_data.width-1, 1, 0, build_data.height-1)
        } else {
