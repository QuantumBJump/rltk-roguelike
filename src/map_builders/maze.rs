...
pub struct MazeBuilder {
    ...
    depth: i32,
    history: Vec<Map>,
    /// HashMap<area ID, Vec<tile index>>
...
impl MazeBuilder {
    ...
    fn build(&mut self) {
        ...
        // Find a starting point; start at the middle & walk left until we hit open space.
        self.starting_position = Position{ x: 2, y: 2 };
        let start_idx = self.map.xy_idx(self.starting_position.x, self.starting_position.y);
        self.take_snapshot();

        // Find all tiles we can reach from the starting point & cull unreachable tiles
        ...
        // Build a noise map for later spawning
